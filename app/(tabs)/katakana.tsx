...
function CharacterCard({ char, romaji }: Character) {
  ...
  return (
    <Animated.View style={{ transform: [{ scale: scaleAnim }] }}>
      ...
      >
        <Text className="text-4xl font-bold text-teal-500">{char}</Text>
...
export default function KatakanaScreen() {
  ...
  return (
    <View className="flex-1 bg-teal-50">
      ...
      >
        {katakanaData.map((group, index) => (
          <TouchableOpacity
            key={group.name}
            onPress={() => setSelectedGroup(index)}
            className={`px-4 py-2 mx-1 rounded-full ${
              selectedGroup === index ? "bg-teal-500" : "bg-white"
            }`}
          >
            <Text
              className={`font-bold ${
                selectedGroup === index ? "text-white" : "text-teal-500"
              }`}
            >
              {group.name}
            </Text>
      ...
      <ScrollView className="flex-1 px-2">
        <View className="flex-row flex-wrap justify-center py-4">
          {katakanaData[selectedGroup].characters.map((char) => (
        ...
        <View className="bg-white rounded-3xl mx-4 p-4 mb-6">
          <Text className="text-xl font-bold text-teal-500 mb-4 text-center">
            คาตากานะทั้งหมด
          </Text>
          <View className="flex-row flex-wrap justify-center">
            {katakanaData.flatMap((group) =>
              group.characters.map((char) => (
                <TouchableOpacity
                  key={char.char}
                  onPress={() => Speech.speak(char.char, { language: "ja" })}
                  className="bg-teal-100 rounded-xl p-2 m-1"
                >
